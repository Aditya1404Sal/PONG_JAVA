    ...
    public void update(double dt) {
        if (vx < 0.0) {
            if (this.rect.x <= this.leftPaddle.x + this.leftPaddle.Width_Rect
               ...
               double diff = (this.rect.x - leftPaddle.x);
               double diff2 = leftPaddle.y - this.rect.y;
                double oldSign = Math.signum(vx);
                vx = newVx * (-1.0 * oldSign);
                vy = newVy;
                vx *= 1 + (diff*values.X_axis_Factor);
                vy *= 1+ (diff2*values.Y_axis_Factor);
            }
        } else if (vx > 0.0) {
            if (this.rect.x + this.rect.Width_Rect >= this.rightPaddle.x
               ...
               double newVx = Math.abs((Math.cos(theta)) * values.BALL_SPEED);
               double newVy = (Math.sin(theta)) * values.BALL_SPEED;
               double diff = rightPaddle.x - this.rect.x;
               double diff2 = rightPaddle.y - this.rect.y;
               double oldSign = Math.signum(vx);
